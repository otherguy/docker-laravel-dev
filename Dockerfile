...
RUN apk --no-cache add --virtual .build-deps $PHPIZE_DEPS \
  ...
  libjpeg-turbo-dev libpng-dev shadow curl curl-dev libxml2-dev icu-dev msmtp openssl-dev autoconf pkgconfig nodejs \
  && docker-php-source extract \
  && docker-php-ext-configure gd --with-freetype-dir=/usr/include/ \
                                 --with-png-dir=/usr/include/ \
                                 --with-jpeg-dir=/usr/include/ \
  && docker-php-ext-install curl dom gd hash iconv intl json mbstring mysqli opcache pdo pdo_mysql session sockets tokenizer xml zip \
